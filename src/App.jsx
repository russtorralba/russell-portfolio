...
function App() {
  return <main>
    ...
    <section className="projects section" id="projects" aria-labelledby="projects-title"><div className="section-heading"><div><p className="eyebrow"><span /> SELECTED WORK</p><h2 id="projects-title">Projects with a<br /><em>purpose.</em></h2></div><p>Six small, thoughtful projects — each made to solve a real need or invite a little curiosity.</p></div><div className="project-grid">{projects.map(([title, type, description, preview, liveDemo], index) => <article className="project-card" key={title}><Preview kind={preview} title={title} /><div className="project-body"><p className="project-number">0{index + 1}</p><h3>{title}</h3><p className="project-type">{type}</p><p className="project-description">{description}</p><div className="card-actions">{liveDemo ? <a className="button small primary" href={liveDemo} target="_blank" rel="noreferrer">View Project ↗</a> : <button className="button small placeholder" disabled>View Project — add URL</button>}</div></div></article>)}</div></section>
    ...
    <section className="contact section" id="contact" aria-labelledby="contact-title"><i className="contact-glow" /><p className="eyebrow"><span /> GET IN TOUCH</p><h2 id="contact-title">Let's build something<br /><em>useful.</em></h2><p>Have a project, website, or idea you'd like to discuss? Feel free to get in touch.</p><div className="contact-actions"><a className="button primary" href="mailto:yourdgtalhub@gmail.com">Email Me</a></div></section>
